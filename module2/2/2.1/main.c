...
// Освобождение строки после использования  
...
int main() {
    ...
    if (load_from_file(&contacts, &contact_count, filename) != 0) {
    ...
    while (1) {
        ...
        int choice;
        scanf("%d", &choice);
        getchar(); //убираем /n из буфера ввода, иначе появлялся прикол Введите фамилию: Введите имя: 
        switch (choice) {
            case 1: {
                ...
                if (add_contact(&contacts, &contact_count, last_name, first_name, middle_name,
                                workplace, position, phone, email, social_link, messenger_profile) == 0) {
            ...
            case 2: {
                ...
                if (edit_contact(contacts, contact_count, id, last_name, first_name, middle_name,
                                 workplace, position, phone, email, social_link, messenger_profile) == 0) {
            ...
            case 3: {
                ...
                if (delete_contact(&contacts, &contact_count, id) == 0) {
            ...
            case 4:
                if (save_to_file(contacts, contact_count, filename) == 0) {
            ...
            case 6: {
                ...
                print_contacts(contacts, contact_count, output, sizeof(output));
                printf("%s", output);
                break;
            }
            case 7:

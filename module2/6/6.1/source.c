...
static int compare_contacts(const Contact* a, const Contact* b) {
    if (a->name.last_name && b->name.last_name) {
        ...
        if (cmp != 0) return cmp;
        if (a->name.first_name && b->name.first_name) {
...
int edit_contact(Node** head, unsigned int id,
    ...
    while (current) {
        if (index == id) {
            ...
            if (last_name_changed || (first_name && strlen(first_name) > 0 &&
                                     strcmp(first_name, current->contact.name.first_name) != 0)) {
                Node* temp = current;
                if (current->prev) {
                ...
                add_contact(head, last_name ? last_name : temp->contact.name.last_name,
                            first_name ? first_name : temp->contact.name.first_name,
                            middle_name ? middle_name : temp->contact.name.middle_name,
                            workplace ? workplace : temp->contact.workplace,
                            position ? position : temp->contact.position,
                            phone ? phone : temp->contact.details.phone,
                            email ? email : temp->contact.details.email,
                            social_link ? social_link : temp->contact.details.social_link,
                            messenger_profile ? messenger_profile : temp->contact.details.messenger_profile);
                free_contact(&temp->contact);
                free(temp);
            } else {
                if (middle_name) {
